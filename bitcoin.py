...
def computeDelta(wt, X, Xi):
    ...
    def similarity(a,b):
    	...
    	sd_b = np.std(b)
    	temp = 0.0
    	sim = sum((a-mean_a)*(b-mean_b))/(len(a)*sd_a*sd_b)
    	# for i in range(0,len(a)):
    	# 	temp += (a[i]-mean_a)*(b[i]-mean_b)
    	# sim = temp/(len(a)*sd_a*sd_b)
    	return sim
    ...
    for i in range(len(Xi)):
    	inter = math.exp(wt*similarity(X[:-1],Xi.iloc[i][:-1]))
    	num += (Xi.iloc[i][-1]*inter)
    	den += inter
